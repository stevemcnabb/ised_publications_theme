...
function MODULENAME_modules_installed($modules) {
    if (in_array('wxt', $modules)) {
      \Drupal::configFactory()
        ...
        ->set('default', 'publications_theme')
        ->set('admin', 'claro')
        ->save(TRUE);
    }
  }
 ...
 */
function publications_theme_theme_suggestions_block_alter(&$suggestions, $variables) {

  // Load theme suggestions for blocks from parent theme.
  foreach ($suggestions as &$suggestion) {
    $suggestion = str_replace('publications_theme_', 'wxt_bootstrap_', $suggestion);
  }
}
